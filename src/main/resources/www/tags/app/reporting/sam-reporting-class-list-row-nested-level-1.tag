<sam-reporting-class-list-row-nested-level-1>
	<guf-linear-layout ref="main-level" orientation="horizontal">
		...
		<div class="class-column flex1">{guf.ancestor(this, 'sam-reporting-class-list-row-nested-level-1').className}</div>
		<div class="total-column">{guf.ancestor(this, 'sam-reporting-class-list-row-nested-level-1').total}</div>
		<div class="tail-column"></div>
	</guf-linear-layout>
	<div if="{guf.ancestor(this, 'sam-reporting-class-list-row-nested-level-1').expanded}" class="nested-container">
		<sam-reporting-class-list-row-nested-level-2 ref="nested-row" each={item in guf.ancestor(this, 'sam-reporting-class-list-row-nested-level-1').item.studentDetails} period="{guf.ancestor(this, 'sam-reporting-class-list-row-nested-level-1').opts.period}"></sam-reporting-class-list-row-nested-level-2>
	</div>
	<style scoped type="dcss">
		...
		:scope > guf-linear-layout[ref="main-level"] > .class-column {
			min-width: 493px;
		}
	...
	<script type="text/javascript">
		...
		function initListEvents() {
			var items = [];
			items = guf.tagsAsArray(tag.refs["nested-row"]);

<sam-teacher-combo-box-item>
	...
	<script>
		var tag = this;
		tag.displayText = opts.data && opts.data.name ? opts.data.name : "";
		tag.defaultDcss = {
			"darktext": "@darktext"
		};
		tag.mixin('mdl');
		...
		tag.maybeSetSelectedOption = function(options) {
			if (options.map(function(item) {return item.email}).indexOf(opts.data.email) != -1) {
				tag.refs["checkbox"].check();
			}
		};

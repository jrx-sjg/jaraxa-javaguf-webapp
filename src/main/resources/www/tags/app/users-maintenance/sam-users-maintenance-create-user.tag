<sam-users-maintenance-create-user>
	<guf-linear-layout ref="container" orientation="vertical" class="container flex1">
		...
		<div class="content">
			<div class="content-title">{guf.i18n.get("app.create_new_user").toUpperCase()}</div>
	...
	<script type="text/javascript">
		...
		var currentInstitution = "", currentEmail, currentRoles, currentLastName, currentFirstName;
		...
		function saveClickHandler() {
			...
			var params = {
				active: true,
				campus: currentInstitution,
				userEmail: emailField.getValue(),
				firstName: firstNameField.getValue(),
				lastName: lastNameField.getValue(),
				roles: selectedRoles
			};
			app.ajax.post("api/users", params
			, function(response, xhr) {
				if(response.success) {
					tag.trigger("created", tag);
				} else {
					guf.console.error("Error saving new user", response);
					saveButton.enable();
				}
			}, function(response, xhr) {
				guf.console.error("Error saving new user", response);

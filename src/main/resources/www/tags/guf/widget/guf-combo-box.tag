<guf-combo-box>
	...
	<guf-menu 
		...
		buttonid="{menuId}" 
		overflow="{overflow}" 
		class="{full-width: fullWidth, overflow: overflow}">
		<guf-menu-item 
			each="{item in parent.comboOptions}">
			<guf-mutable 
				if="{parent.parent.itemTag}" 
				data="{item}" 
				tag-name="{parent.parent.itemTag}">
			</guf-mutable>
			<span 
				if="{!parent.parent.itemTag}">
				{item}
	...
	<style scoped type="dcss">
		...
		:scope > guf-button[ref="current"] > .mdl-button[disabled="disabled"] {
		...
		:scope[disabled="disabled"] > .mdl-label--floating {
			color: @disabledColor;
		}
		:scope:hover > guf-button[ref="current"][disabled="disabled"] > .mdl-button.mdl-button--outline,
		:scope > guf-button[disabled="disabled"] > .mdl-button.mdl-button--outline,
		:scope > guf-button[disabled="disabled"].mdl-label--border-colored > .mdl-button.mdl-button--outline {
			border-color: @disabledColor;
		}
		/*************** End Button ***************/
		...
		/*************** Menu ***************/
		:scope > guf-menu[ref="options"] guf-menu-item {
			flex-shrink: 0;
		}
		:scope > guf-menu[ref="options"].overflow > .mdl-menu__container {
			display: -webkit-box;
			display: -moz-box;
			display: -ms-flexbox;
			display: -webkit-flex;
		...
		:scope > guf-menu[ref="options"].overflow > .mdl-menu__container > .mdl-menu__outline {
			max-height: @maxHeight;
		}
		:scope > guf-menu[ref="options"].overflow > .mdl-menu__container > .mdl-menu {
			display: -webkit-box;
			display: -moz-box;
			display: -ms-flexbox;
			display: -webkit-flex;
			...
			max-height: calc(@maxHeight - 16px);
			overflow-y: auto;
		}
		:scope > guf-menu[ref="options"].full-width > .mdl-menu__container {
			width: 100%;
			display: -webkit-box;
            display: -moz-box;
            display: -ms-flexbox;
		...
		:scope > guf-menu[ref="options"].full-width > .mdl-menu__container > .mdl-menu__outline {
			max-height: @maxHeight;
		}
		:scope > guf-menu[ref="options"].full-width {
			display: -webkit-box;
            display: -moz-box;
            display: -ms-flexbox;
            display: -webkit-flex;
		...
		:scope > guf-menu[ref="options"].full-width > .mdl-menu__container > .mdl-menu {
			display: -webkit-box;
            display: -moz-box;
            display: -ms-flexbox;
            display: -webkit-flex;
		...
		:scope > guf-menu[ref="options"].full-width > .mdl-menu__container > .mdl-menu .mdl-menu__item {
			height: initial;
			min-height: 48px;
		}
		/*************** End Menu ***************/
	...
	<script type="text/javascript">
		...
		function attachItemEvents() {
			var menuItems = tag.refs["options"].tags["guf-menu-item"];
			if (menuItems) {
				for(var j = 0; j < menuItems.length; j++) {
					var item;
					if(tag.itemTag) {
						item = menuItems[j].tags["guf-mutable"];
					} else {
						item = menuItems[j];
					}
		...
		tag.setMenuPosition = function(position) {
			tag.menuPosition = position;
			// update guf-menu tag
			tag.update();
		};

		tag.setOptions = function(options) {
			tag.comboOptions = [];
			tag.update();
			tag.comboOptions = options;
			tag.update();
		};

		tag.setSelectedOptions = function(options) {
			var menuItems = tag.refs["options"].tags["guf-menu-item"];
			if (menuItems) {
				for(var j = 0; j < menuItems.length; j++) {
					var item;
					if(tag.itemTag) {
						item = menuItems[j].tags["guf-mutable"];
						if (item.getTag().maybeSetSelectedOption) {
							item.getTag().maybeSetSelectedOption(options);
						}
					} else {
						item = menuItems[j];
					}

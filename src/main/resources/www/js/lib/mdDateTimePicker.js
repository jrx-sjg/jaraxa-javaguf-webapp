(function (global, factory) {
  ...
  */
		function mdDateTimePicker(_ref) {
			var type = _ref.type,
			    ...
			    _ref$validRanges = _ref.validRanges,
			    validRanges = _ref$validRanges === undefined ? null : _ref$validRanges,
			    _ref$mode = _ref.mode,
			    ...
			    cancel = _ref$cancel === undefined ? null : _ref$cancel,
  ...
  */
		...
		_createClass(mdDateTimePicker, [{
			...
			key: 'setValidRanges',
			value: function setValidRanges(ranges) {
				this._validRanges = ranges || [];
			}
		}, {
			key: 'setPast',
			value: function setPast(newPast) {
				this._past = newPast;
			}
		}, {
			key: 'setFuture',
			value: function setFuture(newFuture) {
    ...
    */
			...
			value: function _buildDialog() {
				...
				if (this._autoClose === !0 && this._ok === null && this._cancel === null) {
				...
				document.getElementsByTagName('guf-canvas').item(0).appendChild(docfrag);
			}
		}, {
			key: '_initTimeDialog',
			...
			key: '_isAllowedDate',
			value: function _isAllowedDate(m) {
				var validRanges = this._validRanges;
				var currentRange;
				if(validRanges === null) {
					return true;
				}
				for(var i=0; i<validRanges.length; i++) {
					...
					if(!(m.isBefore(currentRange.start) || m.isAfter(currentRange.end))) {
						return true;
					}
				}
				return false;
			...
			value: function _initMonth(view, m) {
				...
				var docfrag = document.createDocumentFragment(),
				    ...
				    currentMoment = m.clone().startOf("month");
    ...
    */
				...
				if (this._sDialog.sDate.isSame(m, 'month')) {
					selected = parseInt(this._sDialog.sDate.format('D'), 10);
				...
				for (var i = 0; i < 42; i++) {
					...
					if (i >= firstDayOfMonth && i <= lastDayOfMonth) {
						...
						} else if(!this._isAllowedDate(currentMoment.clone().add(currentDay - 1, 'days'))) {
							cell.classList.add(cellClass + '--disabled');
						} else {
				...
				hourView.onclick = function (e) {
					...
					if (e.target && e.target.nodeName == 'SPAN') {
						...
						if (me._mode) {
							...
							if (me._sDialog.sDate.format('A') === 'AM' && e.target.textContent != "12" ||
								me._sDialog.sDate.format('A') === 'PM' && e.target.textContent == "12") {
     ...
     */
			...
			value: function _changeM() {
				...
				AM.onclick = function (e) {
					...
					AM.classList.add('mddtp-picker__color--active');
				...
				PM.onclick = function (e) {
					...
					PM.classList.add('mddtp-picker__color--active');
				};
			}
		}, {
			key: '_dragDial',
     ...
     */
				fakeNeedleDraggabilly.on('pointerMove', function (e) {
					...
					// GUF Update minutes while dragging
					onDragEnd(e);
				});
				/**
     * netTrek
